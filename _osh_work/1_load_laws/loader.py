...
# loader.py: Use OpenAI API to convert tables and formulas in legal documents into natural language descriptions.
...
class ChineseNumberConverter:
    ...
    def _parse_traditional_chinese(text: str) -> str:
        ...
        try:
            if text == '十':
                return '10'
            elif text.startswith('十') and len(text) > 1:
                ones = ChineseNumberConverter.CHINESE_TO_ARABIC.get(text[1], text[1])
                return str(10 + int(ones)) if ones.isdigit() else text
            elif '十' in text:
                parts = text.split('十', 1)
                tens = ChineseNumberConverter.CHINESE_TO_ARABIC.get(parts[0], '1') if parts[0] else '1'
                ones = ChineseNumberConverter.CHINESE_TO_ARABIC.get(parts[1], '0') if parts[1] else '0'
                return str(int(tens) * 10 + int(ones))
            return text
        except (ValueError, KeyError) as e:
            logger.warning(f"Failed to parse Chinese number '{text}': {e}")
            return text
...
class OpenAIConverter:
    """Core Logic: Uses OpenAI API to convert tables and formulas to natural language"""
...
class PDFProcessor:
    ...
    def _ocr_page(self, page) -> str:
        ...
        try:
            ...
            if not text.strip():
                logger.warning("OCR returned empty text; page may be image-only or corrupted.")
            return text
            
        except Exception as e:
            logger.error(f"OCR error: {e}")
            raise RuntimeError("OCR failed; ensure Tesseract is installed and configured.") from e
...
class ChunkManager:
    ...
    MAX_CHUNK_SIZE: Final = 1500  # characters
    ...
    def estimate_tokens(text: str) -> int:
        ...
        try:
            encoding = tiktoken.encoding_for_model("gpt-4")  # Adjust model as needed
...
def main():
    ...
    INPUT_FOLDER = os.getenv("INPUT_FOLDER", "./input")  # Default to relative path if not set
    OUTPUT_FOLDER = os.getenv("OUTPUT_FOLDER", "./processed_output")
