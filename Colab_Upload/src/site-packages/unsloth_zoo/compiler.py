...
def create_new_function(
    ...
    if os.path.exists(file_location):
        print(f"Unsloth: Using cached module {name} from {file_location}")
    ...
    if overwrite or not os.path.isfile(file_location):
        while not os.path.isfile(file_location):
            ...
            time.sleep(0.01)
    pass
    print("stop overwriting, using local cache")
    ...
    print(f"Unsloth: Loading module {name} from {file_location}")
    module_name = f"unsloth_cache_{name}"
    spec = importlib.util.spec_from_file_location(module_name, file_location)
    new_module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = new_module
    ...
    if new_module is None:
        raise ImportError(f'Unsloth: Cannot import {file_location}')
